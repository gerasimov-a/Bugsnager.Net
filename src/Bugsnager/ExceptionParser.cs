...
//                                        .Where(x => !x.Method.StartsWith("Bugsnag.")).ToList();
        ...
        private static IEnumerable<StackTraceFrameInfo> GenerateStackTraceInfo(string callTrace)
        {
            var regex = new Regex(@"at (?<method>.*)");

            var emptyStack = new StackTraceFrameInfo
            {
                File = "No stack info provided",
                LineNumber = 0
            };

            IList<StackTraceFrameInfo> stackFrames = new List<StackTraceFrameInfo>();
            if (String.IsNullOrWhiteSpace(callTrace))
            {
                stackFrames.Add(emptyStack);
                return stackFrames;
            }

            var matches = regex.Matches(callTrace);
            ...
            foreach (Match match in matches)
            {
                if (match.Success)
                {
                    ...
                    stackFrames.Add(new StackTraceFrameInfo
                    {
                        File = file,
